...
impl UIContext {
    ...
    pub(crate) fn render(&mut self, _renderer: Rc<RefCell<dyn Renderer2D>>) {
        self.end_frame();
        ...
        for (_, state) in &self.state {
            match &state {
                DrawableType::Panel(panel) => panel.render(_renderer.clone(), &self.default_font),
                DrawableType::Screen(screen) => screen.render(_renderer.clone()),
            }
        }
